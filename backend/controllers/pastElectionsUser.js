...
const pastElectionsUser = async (req, res) => {
  try {
    const { userId } = req.body;
    const user = await User.findById(userId);
    if (!user) return res.status(404).json({ message: "User not found" });

    const communityKeys = user.communities.user.map((c) => c.community_key);
    ...
    const result = await Promise.all(
      communityKeys.map(async (key) => {
        ...
        return {
          ...
          elections // results are part of the election object
        };
      })
    );
    ...
    console.error("❌ Error fetching past elections for user:", err);
    res.status(500).json({ message: "Internal server error" });
  }
};

export default pastElectionsUser;
